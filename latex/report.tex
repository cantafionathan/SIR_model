...
None of the higher order effects were statistically significant at the $5\%$ level. Referring to Table~\ref{tab:anova}, \verb`soc.iso`, \verb`quar.dur`, and \verb`num.daily` all have statistically significant effects on \verb`load` at the $0.1\%$ level. While \verb`rate.vac` is only significant at the $10\%$ level.
