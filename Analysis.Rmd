...
data$soc.iso <- ordered(data$soc.iso)
data$rate.vac <- ordered(data$rate.vac)
data$quar.dur <- ordered(data$quar.dur)
data$num.daily <- ordered(data$num.daily)
contrasts(data$soc.iso) <- contr.poly(levels(data$soc.iso))
contrasts(data$rate.vac) <- contr.poly(levels(data$rate.vac))
contrasts(data$quar.dur) <- contr.poly(levels(data$quar.dur))
contrasts(data$num.daily) <- contr.poly(levels(data$num.daily))
lm_poly <- lm(load ~ soc.iso + rate.vac + quar.dur + num.daily, data = data)
summary(lm_poly)
